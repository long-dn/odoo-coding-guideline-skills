...
Only create directories the module actually needs. Keep Python model code in `models/`, transient models in `wizard/`, report model code and report XML in `report/`, business/security data in `data` and `security`, translations in `i18n`, and UI assets in `static/src`.

Do not create `i18n/*.po` by hand during coding tasks. Translation files in `i18n` should be generated by Odoo's translation export workflow so they contain the exact extracted terms and avoid wasteful or stale entries. Load `translations.md` before touching `i18n/`.
